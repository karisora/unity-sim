...
internal class ROS2ForUnity
{
    ...
    public void CheckIntegrity()
    {
        ...
        if (!IsStandalone() && string.IsNullOrEmpty(ros2SourcedCodename)) {
            Debug.LogError(
                "ROS2 environment is not sourced for this non-standalone ros2-for-unity build. " +
                "Source ROS2 " + ros2FromRos2csMetadata + " before launching Unity."
            );
        }

        if(!IsStandalone() && !string.IsNullOrEmpty(ros2SourcedCodename) && ros2SourcedCodename != ros2FromRos2csMetadata) {
            Debug.LogError(
                "ROS2 version in 'ros2cs' metadata (" + ros2FromRos2csMetadata +
                ") doesn't match currently sourced version (" + ros2SourcedCodename + "). " +
                "Source the matching ROS2 environment before launching Unity."
            );
        }
    ...
    private string ResolveMetadataPath(string expectedPath, string fileName)
    {
        ...
        string ros2ForUnityPath = GetRos2ForUnityPath();
        if (Directory.Exists(ros2ForUnityPath))
        {
            string[] matches = Directory.GetFiles(ros2ForUnityPath, fileName, SearchOption.AllDirectories);
            if (matches.Length > 0)
            {
                Debug.LogWarning("Using ROS2 metadata from fallback path: " + matches[0]);
                return matches[0];
            }
        }

        throw new System.IO.FileNotFoundException(
            "Could not find " + fileName + ". Expected path: " + expectedPath
        );
    }
    ...
    private void LoadMetadata() 
    {
        ...
        try
        {
            string ros2csMetadataPath = ResolveMetadataPath(
                GetPluginPath() + separator + "metadata_ros2cs.xml",
                "metadata_ros2cs.xml"
            );
            string ros2ForUnityMetadataPath = ResolveMetadataPath(
                GetRos2ForUnityPath() + separator + "metadata_ros2_for_unity.xml",
        ...
        {
#if UNITY_EDITOR
            var errMessage = "Could not find ROS2 metadata files. " + e.Message;
            Debug.LogError(errMessage);
            EditorApplication.isPlaying = false;
            throw new System.IO.FileNotFoundException(errMessage, e);
#else
            const int NO_METADATA = 1;
            Debug.LogError("Could not find ROS2 metadata files. " + e.Message);
