...
public class ROS2UnityComponent : MonoBehaviour
{
    ...
    public bool Ok()
    {
        lock (mutex)
        {
            ...
            return (nodes != null && ros2forUnity != null && ros2forUnity.Ok());
        }
    }
    ...
    public ROS2Node CreateNode(string name)
    {
        ...
        lock (mutex)
        {
            if (quitting || nodes == null || ros2csNodes == null)
            {
                throw new InvalidOperationException("Cannot create ROS2 node while ROS2UnityComponent is shutting down or not initialized.");
            }
    ...
    public void RemoveNode(ROS2Node node)
    {
        lock (mutex)
        {
            if (node == null || nodes == null || ros2csNodes == null)
                return;
    ...
    public void RegisterExecutable(Action executable)
    {
        ...
        lock (mutex)
        {
            if (quitting || executableActions == null)
                return;
    ...
    private void Tick()
    {
        while (!quitting)
        {
            if (Ok())
            {
                lock (mutex)
                {
                    if (quitting || executableActions == null || ros2csNodes == null)
                    {
                        return;
                    }
